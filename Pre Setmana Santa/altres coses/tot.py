...
def simulacio_2D(id_passadis, m, n, num_iteracions, aforament, entrada_unica=False, entrades_laterals=False):
    ...
    for t in range(num_iteracions):
        ...
        ind_in_passadis = [ind for ind in individus if ind.get_recorregut()[-1] != None]
        nous_individus = random.randint(0, aforament-len(ind_in_passadis))
        print(f"Quantitat d'invididus en t = {t} = {len(ind_in_passadis)}\n")
        ...
        for j in range(nous_individus):
            ...
            individus.append(Individu(id_individu, entrada, objectiu, velocitat, m, n))
