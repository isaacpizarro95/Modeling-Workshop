...
def moure_individu(individu, passadis):
    ...
    for i in range(individu.get_velocitat()):
        ...
        if (objectiu[0] != 0):
            ...
            posicions = [(x,y),(x-1, y), (x, y-1), (x, y+1), (x-1, y+1), (x-1, y-1), (x+1, y+1), (x+1, y-1)]
        #posicions = [(x,y), (x-1, y), (x+1, y), (x, y-1), (x, y+1), (x-1, y+1), (x+1, y+1), (x-1, y-1), (x+1, y-1)]
        posicions_valides = [pos for pos in posicions if 0 <= pos[0] < passadis.get_m() and 0 <= pos[1] < passadis.get_n() and matriu[pos] != 2 and matriu[pos] != 1 and matriu[pos] != 4]
        ...
        for pos in posicions:
            if 0 <= pos[0] < passadis.get_m() and 0 <= pos[1] < passadis.get_n() and matriu[pos] != 2 and matriu[pos] != 4:
                if matriu[pos] == 1:
                    ...
                    #puntuacio = colisions * p_colisions - distancia_objectiu * p_distancia - canvi_direccio * p_canvi
